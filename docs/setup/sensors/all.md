...
???+ danger "Note"
    The built-in **GPU Temp** sensor in HASS.Agent is no longer functional.
    It has been set to always return `0` for backwards compatibility because the underlying library was removed due to security concerns.
    Use the WMI Query Sensor as shown above to restore this functionality.
